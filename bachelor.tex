...
Java 1.5 extended the \inline{for} loop syntax to add a variant that can do the
iterator management automatically, such a statement is called the
...
With the addition of generics in Java 1.5, the \inline{next} method was changed
to return an element of the containing type instead of an Object that needed to
be cast to the desired type, thus improving type safety.
...
We saw the need for a better support for data compression in OCaml, so for this
thesis, we made it possible for OCaml programmers to use a wide range of
compression algorithms. The algorithms are known to be correct since libarchive
usually uses the reference implementations of these algorithms, where such
implementations exist.

While implementing the binding, a number of shortcomings in libarchive itself
were identified and patched. Thus this thesis has not only improved the
situation for users of OStreamer, but all other users of libarchive can profit
from the improvements in libarchive introduced as a product of this thesis.
