...
% the highlighter fails sometimes and draws an ugly box around things he does
% not understand. remove the box by redefining the style to be 'nothing'
\makeatletter
  \def\PY@tok@err{}
\makeatother
...
without having to evaluate all values of it. These values need only be
evaluated when the head of the sequence is requested.

OCaml is currently missing a composable implementation of streams: the Stream
module from the OCaml Standard Library is essentially an imperative API and
uses exceptions whereas we are looking for an API that is more functional in
spirit: The objective in this bachelor's thesis is to create an alternative API
replacing exceptions with option types\footnote{We will actually use the error
monad, introduced in \autoref{sec:errormonad}} which eliminate the danger of
unchecked exceptions raising at runtime and at the same time improve
composability. The effectiveness of this stream library is then demonstrated by
building a library for data compression on top of it.
...
\begin{listing}[H]
  ...
  \caption{Reading file line by line in C, \inline{fgetline()} inspired by
	  \inline{getline(3)}~\cite[p.~165]{kernighan1988c}}
  \label{lst:creadfile}
\end{listing}
...
compressed stream comes out with the compression function as a black box. Such
...
Libraries for these algorithms are often implemented in C (\inline{zlib} for
gzip, \inline{libbz2} for bzip2) or at least have an interface to be used from
C (LZMA reference implementation\footnote{7-Zip project,
\url{http://www.7-zip.org/}}), therefore giving implementers in C a wide
variety to choose from. Unfortunately, these libraries all provide different
APIs, so just switching the compression algorithm from gzip to bzip2 is not
very convenient.  This problem is adressed by
...
Fortunately, since the C application binary interface is mostly standarized and
the calling conventions are both simple and commonly understood, most
programming languages provide interfaces to call C code, thus implementing the
compression algorithms in C is the most common denominator for a range of
languages. These interfaces can range quite different: from compatibility and
linking directly to C code like C++ does, to dynamically loading the C
libraries on runtime via \inline{dlopen} like Python. OCaml goes a middle way,
by requiring glue code in C that interfaces the C libraries and returns OCaml
data types to the OCaml run time.  These functions are then fitted with OCaml
type signatures and can then be directly called from the code as if they were
written in OCaml as well.
...
for (Iterator i = stamps.iterator(); i.hasNext(); ) {
	Stamp s = (Stamp) i.next(); // might throw ClassCastException
    ... // Do something with the stamp
}
  \end{minted}
  \caption{\enquote{Classic} Java style iteration, not type safe and without
	  syntactic sugar~\cite[p.~110]{bloch2008effective}}
  \label{lst:jiter4}
\end{listing}
...
iterating~\cite[p.~653]{horstmann2007core}.
...
for (Iterator<Stamp> i = stamps.iterator(); i.hasNext(); ) {
  ...
  \end{minted}
  \caption{Type safe iteration without syntactic
    sugar~\cite[p.~111]{bloch2008effective}}
  \label{lst:jiter}
\end{listing}

With the addition of generics in Java 1.5~\cite[p.~109]{bloch2008effective},
the \inline{next} method was changed to return an element of the containing
type instead of an \inline{Object}, as seen in \autoref{lst:jiter}. This
improves type safety, as the contents of the collection can now be statically
tracked by the compiler and illegal accesses can be detected at compile time.
...
\enquote{enhanced \inline{for} statement}~\cite[p.~309]{gosling2013java}. This
type of \inline{for} loop creates the iterator on its own, as in
\autoref{lst:jiter5}.
...
for (Stamp s : stamps) { // No cast
  ...
  \end{minted}
  \caption{Java 1.5-style iteration with syntactic
    sugar~\cite[p.~111]{bloch2008effective}}
  \label{lst:jiter5}
\end{listing}
...
\inline{Reader} and \inline{Writer} classes~\cite[p.~254]{flanagan2005java}.
...
streams, most notably the \inline{FileInputStream} and
\inline{FileOutputStream} classes, which allow reading from files in the file
system.

To read a file, either the name of the file can be supplied to a
\inline{FileInputStream} or a Java \inline{File} instance. The latter
represents a file in the file system itself~\cite[p.~252]{flanagan2005java} and
not a handle to an opened file like \inline{FILE} in C. After creating an
instance of the \inline{FileInputStream}, data can be read via its
\inline{read} method which is very similar to the familiar \inline{fread}
function from C: it reads a number of bytes into an array of byte-type at an
optional offset and returns the number of bytes that were read.
...
Python is a dynamically-typed high level language designed in the the 1990ies
and enjoying constant improvement. It has seen rising adoption in the last few
years in scientific communities as well as for web
development~\cite{copeland2010growth}. This adoption could be attributed to the
simplicity of the language and the thriving ecosystem\footnote{The Python
Package Index \url{https://pypi.python.org/} lists over 32000 packages
(as of mid-2013)}.
...
specific behaviors: \emph{If it walks like a duck and quacks like a duck, then
it must be a
duck}~\cite[p.~89]{olsen2011eloquent}\cite[p.~51]{clinton2009ruby}. The term
duck-typing was coined by the community of the Ruby programming language but
...
\begin{itemize}
  ...
  \item Throw a \inline{StopIteration} exception after the last element was
	  ...
	  will not provide any more values~\cite[p.~345]{lutz2009learning}.

\end{itemize}

There is an additional indirection by the means of the \inline{\_\_iter\_\_}
method: as an iterator is created e.g. by a for loop construct, the runtime
calls the \inline{iter} built-in function with the object which in turn calls
the objects \inline{\_\_iter\_\_} method which is expected to return an
\emph{Iterable} that supports the aforementioned
interface~\cite[p.~356]{lutz2009learning}. This iterable can be the object
itself, as shown in the code listings.
...
\begin{listing}[H]
  \inputminted[linenos]{python}{squares.py}
  \caption{An iterable in Python~\cite[p.~714]{lutz2009learning}}
  \label{lst:pyiterable}
\end{listing}

\autoref{lst:pyiterable} shows a very simple implementation of an iterable
object. The object returns a finite number of values, although infinite
iterables are also possible as per \autoref{lst:pyiterableinfinite}.

\begin{listing}[H]
  \inputminted[linenos]{python}{iterable.py}
  \caption{An infinite iterable, slicing off the first 5 elements}
  \label{lst:pyiterableinfinite}
\end{listing}
...
generator~\cite[p.~714]{lutz2009learning}. \autoref{lst:pygenerator}
demonstrates a simple generator in action.
...
\inline{fopen} in C~\cite[p.~326]{chun2006core}. The type of this object is
\inline{file} which explains the naming of \enquote{file-like} objects.
...
default is to read the data until the end~\cite[p.~329]{chun2006core}. The
\inline{write} method takes a string\footnote{Python 2.x uses the
\inline{str}-bytestring type, Python 3.x uses either the \inline{str} Unicode
string type or \inline{bytes} type} and writes it to the file. The interface is
very simple but for real files it can also be extended by adding methods to
seek inside of a file, getting the inode number of the file etc.
...
and prints its contents. This object could also be passed to most functions
that expect to work with files and work just as well.
...
accessible to the C code, so we can work with the information contained in
the OCaml values.
...
not only frees memory but also reorders the memory to minimize fragmentation in
a process called \enquote{compaction}} that holds the value, it needs to update
the pointer that \inline{unit} points to, therefore such a variable needs to be
registered to the GC first. In this example, the value passed is an instance of
the OCaml unit type, which does not hold information that would require being
extracted. Yet it is required, since OCaml functions are required to have at
least one argument, so it is customary to pass an argument of unit type.
...
collector since when the program terminates all memory is freed automatically,
...
With the low level interface implemented, we had a way to do compression and
...
We will improve the interface  by adding more information to the types by using
phantom types, described in \autoref{sec:phantom}. Also, apart from programmer
errors during programming, there might be errors happening at run time: the
archive that is supposed to be read does not exist, the archive file is corrupt
or maybe the data can't be written because the disk is full.  For this, C code
commonly uses special return values. The downside of this approach is that the
programmer might forget to check the return value, so the program continues to
run despite the error causing even more problems and \enquote{weird} behaviour.
...
The \inline{archive} type can be thought of as a file handle, similar to
\inline{FILE} from C. Similar to the C type, it can be both open and closed. In
addition, libarchive makes a difference between \enquote{read} and
\enquote{write} \inline{archive} handles: \enquote{read} handles are used for
opening and extracting files from an archive and are created with a specific
API call, \enquote{write} handles are used to create archives and have their
own specific creation function. For simplicity, libarchive does not encode this
difference into the type of the handles as both handle types are of type
...
on creation and \enquote{populated} after feeding in the compressed data.
...
untagged \inline{float} representation at run
time~\cite{hinze2003fun}.
...
This section describes the technical details on how to install and use
OStreamer and what APIs are currently available as well as how they are used.
For an even more in-detail view, the reader is encouraged to view the source of
OStreamer and to read the documentation of libarchive.

\subsection{API}
\label{sec:api}

It is best to describe the lower level API first, since to understand the
inner workings of the higher level API the details might be helpful. We start
with the definition of the libarchive handles, but unlike libarchive these are
of distinct types and can be annotated with extra values, they are called
\inline{r} for read handles and \inline{w} for write handles. Short names were
chosen, since the user of OStreamer only needs to pass these around, never to
create them. For the initialization of such a handle, the programmer has to
call \inline{read\_new\_configured} or \inline{write\_new\_configured} with a
list of filters to select the compression type.
...
The resulting value is an \enquote{empty} read handle, since no compressed data was yet
fed into libarchive that could be decompressed. Similarly, the write handle is
\enquote{closed}, since there was no space allocated to write the data to.

The filters that are possible for reading files are listed below, most notably
the \inline{AllFilterReader} which reads all formats which libarchive is able
to recognize. The only exception is the \inline{NoneFilterReader}, which is
required to read archives that are not wrapped in a filter, e.g. when reading
TAR files that are not compressed. More than one filter can be selected.

\begin{multicols}{3}
  \begin{itemize}[nolistsep]
    \item \texttt{AllFilterReader}
    \item \texttt{BZip2FilterReader}
    \item \texttt{CompressFilterReader}
    \item \texttt{GRZipFilterReader}
    \item \texttt{GZipFilterReader}
    \item \texttt{LRZipFilterReader}
    \item \texttt{LZipFilterReader}
    \item \texttt{LZMAFilterReader}
    \item \texttt{LZOPFilterReader}
    \item \texttt{NoneFilterReader}
    \item \texttt{RPMFilterReader}
    \item \texttt{UUFilterReader}
    \item \texttt{XZFilterReader}
  \end{itemize}
\end{multicols}

The possible formats for reading include all formats that libarchive supports.
\inline{AllFormatReader} reads all formats, except for uncompressed files. An
example of such data are files that are only compressed using \emph{GZip}.
These need to be read using the \inline{RawFormatReader}, because the
decompression is already handled by the filter.

\begin{multicols}{3}
  \begin{itemize}[nolistsep]
     \item \texttt{SevenZipFormatReader}
     \item \texttt{AllFormatReader}
     \item \texttt{ARFormatReader}
     \item \texttt{CABFormatReader}
     \item \texttt{CPIOFormatReader}
     \item \texttt{GnuTARFormatReader}
     \item \texttt{ISO9660FormatReader}
     \item \texttt{LHAFormatReader}
     \item \texttt{MtreeFormatReader}
     \item \texttt{RARFormatReader}
     \item \texttt{RawFormatReader}
     \item \texttt{TARFormatReader}
     \item \texttt{XARFormatReader}
     \item \texttt{ZipFormatReader}
  \end{itemize}
\end{multicols}
...
\begin{multicols}{3}
  \begin{itemize}[nolistsep]
    \item \texttt{Base64FilterWriter}
    \item \texttt{BZip2FilterWriter}
    \item \texttt{CompressFilterWriter}
    \item \texttt{GRZipFilterWriter}
    \item \texttt{GZipFilterWriter}
    \item \texttt{LRZipFilterWriter}
    \item \texttt{LZipFilterWriter}
    \item \texttt{LZMAFilterWriter}
    \item \texttt{LZOPFilterWriter}
    \item \texttt{NoneFilterWriter}
    \item \texttt{UUFilterWriter}
    \item \texttt{XZFilterWriter}
  \end{itemize}
\end{multicols}

The same goes for the write format, it has to be chosen manually. There can
only be one write format chosen per handle.

\begin{multicols}{3}
  \begin{itemize}[nolistsep]
    \item \texttt{SevenZipFormatWriter}
    \item \texttt{ARBSDFormatWriter}
    \item \texttt{ARSVR4FormatWriter}
    \item \texttt{CPIOFormatWriter}
    \item \texttt{CPIONEWCFormatWriter}
    \item \texttt{GnuTARFormatWriter}
    \item \texttt{ISO9660FormatWriter}
    \item \texttt{MtreeFormatWriter}
    \item \texttt{PAXFormatWriter}
    \item \texttt{RawFormatWriter}
    \item \texttt{SharFormatWriter}
    \item \texttt{USTARFormatWriter}
    \item \texttt{V7TARFormatWriter}
    \item \texttt{XARFormatWriter}
    \item \texttt{ZipFormatWriter}
  \end{itemize}
\end{multicols}
...
Only from this state can data be extracted, which happens with
\inline{extract\_all}.
...
The result of the call to \inline{extract\_all} is a list of
\inline{ost\_entry} types wrapped in an error monad. The error monad can either
contain this list or a error message containing the reason why extraction of
these files failed (unknown format, corrupted archive).
\inline{entry\_metadata} closely matches the \inline{entry} type of libarchive
itself and represents the meta data of an item that can be contained in an
archive:
...
Some of these values are optional, thus represented as option types. The file
type gets translated from C to the \inline{Unix.file\_kind} type in OCaml. Most
common values are \inline{S\_REG} for regular files and \inline{S\_DIR} for
directories. The content of regular files is represented as an union type,
\inline{ost\_entry}, which is useful when iterating over archive entries since
it forces the programmer to handle all defined file types, thus potentially
improving the quality of programs since they need to handle these cases.

When writing, an analogous set of operations exist. A \enquote{closed} handle
has to be converted into an \enquote{opened} handle, which is done by
\inline{write\_open\_memory}, which allocates an automatically managed region
of memory into which the archive can be written. To write a single entry to the
archive, there exists the function \inline{write\_close} which takes a write
handle and an entry and returns the write handle. The write handle is not
changed but returning the handle improves composability, since it can be
combined in accumulating functions like \inline{fold\_left}.
...
The data is written in a lazy fashion by libarchive, so to retrieve the
compressed archive, the write handle has to be closed, using
\inline{write\_close}, which returns either the compressed data or an error
message via the error monad type.

This concludes the low level API. From this point the high level API was
constructed and is a lot simpler to use.

\begin{minted}[linenos]{ocaml}
val construct: 'a -> 'a ErrorMonad.t
val compress: write_format -> write_filter list -> ost_entry list -> string ErrorMonad.t
val decompress: string -> ost_entry list ErrorMonad.t
val (|>): 'a ErrorMonad.t -> ('a -> 'b ErrorMonad.t) -> 'b ErrorMonad.t
\end{minted}

A typical use of the API constructs a pipe of operations to be performed. The
data to be processed is fed into the pipe using \inline{construct}, then zero
or more operations like \inline{decompress} or \inline{compress} can be chained
using the \inline{|>} operator, very similar to how data can be processed in
Unix shells. Ath the end of the processing, an \inline{ErrorMonad} type is
returned which can be pattern matched to get the resulting value, or in the
case of an error, the error message. A sample usage of the high level API shown
in in \autoref{sec:usage}.

\subsection{Usage}
\label{sec:usage}

To install OStreamer a number of prerequisites need to be installed:

\begin{itemize}
  \item OCaml, at best a version with native compilers (\texttt{ocamlopt.opt})
	  tested with OCaml 4.00.1
  \item OASIS, tested with 0.3.0
  \item libarchive, a version newer than 3.1.2, e.g. the development version
	  including commit \texttt{3ae99fbc24}. Development packages (if any)
	  also need to be installed.
\end{itemize}

If the operating system or distribution ships an outdated version of
libarchive, it can also be installed in a separate location. In that case, some
adjustments need to be done to use the newer libarchive, in particular setting
the GCC options to pick up the proper include files and to link agains the
proper shared objects that include the patches written as part of this thesis.
This issue will go away in the future when a new release of libarchive is made
and included in the newest versions of the operating systems.

The normal installation instructions are shown in \autoref{lst:compiling},
along with a call to the unit test to verify the code works properly.
...
./unittest.native
......
Ran: 6 tests in: 0.02 seconds.
OK
  \end{minted}
  \caption{Compilation and testing of OStreamer}
  \label{lst:compiling}
\end{listing}

After installation, both low level as well as high level APIs can be used. For
increased type safety and easier usage, the high level API demonstrated in
\autoref{lst:usage} is recommended.

\begin{listing}[H]
  \begin{minted}[linenos]{ocaml}
let compress_roundtrip _ =
  let (|>) = Pipe.(|>) in
  ...
  | ErrorMonad.Success (entries) -> (match entries with
    | [Archive.File (c, m)] -> print_endline (String.concat " " ["Contents:"; c])
    | [r] -> print_endline "Not handling other types of entries"
    | x::xs -> print_endline "Not handling multiple entries"
    | [] -> print_endline "Not handling zero entries")
  | ErrorMonad.Failure (errcode, errmsg) -> print_endline "Operation failed"
  \end{minted}
  \caption{Usage of OStreamer's high level API}
  \label{lst:usage}
\end{listing}

Line~2 defines a local alias for the \inline{|>} operator, which makes it
possible to use it with infix syntax in the following code. These aliases can
be also outside of the functions and can also be used by using \inline{open
Pipe} to include the contents of the whole module. In line~3 a convenience
function to generate \inline{ost\_metadata} is used and line~4 constructs an
\inline{ost\_entry} type representind a file with the content
\texttt{"contents"} and the file name \texttt{filename}. Line 5 creates a
curried function for convenience — \inline{compress} is specialized with a
\inline{RawFormatWriter} and \inline{GZipFilterWriter}, thus compressing the
file using plain \emph{GZip} without a container format.

The most interesting part is line~6 which constructs a pipeline. First it wraps
the single file, represented by a list with one element containing
\inline{entry} into the pipeline type. Then, it compresses the files into a
\emph{GZip} file. This file is then decompressed (using automatic file type
detection) by \inline{Pipe.decompress}. The result can be either a
\inline{Failure} in which case an error message will be printed or a
\inline{Success} in which case the result has to be evaluated. If it is a file,
its content will be printed, otherwise a message is shown why this file could
not be printed.
