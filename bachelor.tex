...
It is best to describe the lower level API first, to better understand the
inner workings of the higher level API. We start
...
The resulting value is an \enquote{empty} read handle, since no compressed data
was yet fed into libarchive that could be decompressed. Similarly, the write
handle is \enquote{closed}, since there was no space allocated to write the
data to.

The available \inline{read\_filter} filters that are possible for reading files
are listed below, most notably the \inline{AllFilterReader} which reads all
formats which libarchive is able to recognize. The only exception is the
\inline{NoneFilterReader}, which is required to read archives that are not
wrapped in a filter, e.g. when reading TAR files that are not compressed. More
...
improving the quality of programs since they need to handle these cases. In the
case of a file, the \inline{ost\_entry} constructor consists of a tuple of the
file's contents, represented as byte array of type \inline{string} as well as
the meta data. A directory entry only consists of the meta data.
...
\inline{write\_close}, which returns either the compressed data as a byte array
of type \inline{string} or an error message, both via the error monad type.

This concludes the low-level API. From this point the high-level API was
constructed and is a lot simpler to use. It should be preferred in typical
code, unless there is a specific need for the lower level functions.

\subsection{High-level API}
\label{sec:hlapi}

The high-level API consists of just a few functions. In particular, all handle
types are abstracted away, only the format, filter and entry types remain. At
this level, the \inline{ErrorMonad} types are introduced.
...
case of an error, the error message. A sample usage of the high level API is
shown in \autoref{sec:usage}.
...
\begin{itemize}
  ...
  \item libarchive, a version newer than 3.1.2, e.g. the development version
    including commit \texttt{3ae99fbc24}. Development packages (if any) also
...
the GCC options to pick up the proper include files and to link against the
...
After installation, both low level as well as high level APIs are available. For
...
let compress_roundtrip _ =
  let (|>) = Pipe.(|>) in
  let meta = Archive.generate_metadata "myfile.txt" in
  let entry = Archive.File ("Hello Real World!", meta) in
