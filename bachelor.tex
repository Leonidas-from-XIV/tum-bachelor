...
There is a number of filters for reading that can be selected, most notable the
\inline{AllFilterReader} which reads all formats which libarchive is able to
recognize. The only exception is the \inline{NoneFilterReader}, which is
required to read archives that are not wrapped in a filter, e.g. when reading
TAR files that are not compressed.
...
Again, there is support for all formats that libarchive supports.
\inline{AllFormatReader} reads all formats, except for uncompressed files like
files that are only compressed using \emph{GZip}. These need to be read using
the \inline{RawFormatReader}, because the decompression is already handled by
the filter.
...
It is useful to describe the lower level API first, since to understand the
...
This concludes the low level API. From this point the high level API was constructed and is a lot simpler.
...
% TODO: merge in the part I wrote about the formats and filters
