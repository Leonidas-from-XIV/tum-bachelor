...
and Python, since in Java there are distinct facilities for reading and writing
whereas C just uses a \inline{FILE} that is opened in a reading or writing
mode, rendering write operations on a read-handle invalid during program
execution. Python's file-like objects have adopted similar semantics. In Java,
these types of handles are distinguished at compile-time.

The Java standard library supplies a number of different types of these
streams, most notably of course the
\inline{FileInputStream}/\inline{FileOutputStream} classes, which allow reading
...
The first action of a function is to use the \inline{CAMLparam}
macro\footnote{In particular \inline{CAMLparam1} which means that it takes only
one argument} to register the parameter of the function, \inline{unit} to the
garbage collector as being in use, so it is not allowed to collect it just yet.
In addition, if the garbage collector decides to move the memory
block\footnote{the OCaml garbage collector not only frees memory but also
reorders the memory to minimize fragmentation in a process called
\enquote{compaction}} that holds the value, it needs to update the
pointer that \inline{unit} points to.
...
Another issue arises when exposing pointers to C objects to OCaml code: the
user has to take case of freeing the objects. For this problem, there are two
possible solutions: either expose C functions to OCaml that handle freeing the
allocated resources when the OCaml programmer knows he won't be using them
furthermore or putting these objects under the control of the garbage collector
of OCaml.
...
\begin{itemize}
  ...
  \item Implicit garbage collection has the advantage of never forgetting to
    ...
    collector decides to free the memory.
\end{itemize}

For the binding, it was decided to be consistent with the OCaml language and
choose the safer route, that is manage the objects with the garbage collector
to rule out crashes due to incorrect memory management.
...
predefined blocks like integers and floating point numbers. But to manage C
...
This thesis does better by splitting the handle in two distinct types, so
...
the \inline{external} definition that the return value of the
\enquote{read} handle creation function is \inline{readhandle} and not
\inline{archive}. All functions that would take C
\inline{archive} \enquote{read} handles can be declared to take OCaml
\inline{readhandles} instead. \enquote{write} handles and their functions are
...
could lead in the future and of course, what insights were gained in the
process.

\section{Improvements}
\label{sec:usage}

The beginning of this thesis was an issue with camlzip not being able to handle
archives in-memory thus forcing users of camlzip to write files to disc first
which creates a lot of redundant I/O and going through a hard disc potentially
reduces performance. In case of solid state drives, this creates additional
unnecessary writes, which decrease the life time of the disc.

This bug could have been amended, but it would only solve the problem with ZIP
files, therefore a more general method is to be preferred. This more general
method has been a more generic binding TODO.

One of the advantages of OCaml is that it is very expressive, so while there
was much consideration about the API and implementation, it ended up being
rather few lines compared to the C code which spends a lot of lines in managing
memory thus dominating the line count.
