...
\inline{Reader} and \inline{Writer} classes~\cite{flanagan2005java}.
...
represents a file in the file system itself~\cite{flanagan2005java} and not a
handle to an opened file like \inline{FILE} in C. After creating an instance of
the \inline{FileInputStream}, data can be read via its \inline{read} method
which is very similar to the familiar \inline{fread} function from C: it reads
a number of bytes into an array of byte-type at an optional offset and returns
...
specific behaviors: \emph{If it walks like a duck and quacks like a duck,
then it must be a duck}~\cite{lutz2013learning,olsen2011eloquent,clinton2009ruby}.
