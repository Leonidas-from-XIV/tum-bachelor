...
introduces new polymorphic variant types to implement additional annotations.
The code in \autoref{lst:phantom} adds an annotation in float types on which
unit the float value represents and prevents programmers from mixing meters
with feet, an error that can be easily done and is hard to detect.
